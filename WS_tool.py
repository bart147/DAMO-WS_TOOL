...
# v1.4 notes
# 03-11-2016, B. Kropf
# Parameter sys.argv[4] verbetert: ook in checks verderop toegevoegd als voorwaarde.
# .upper() toegevoegd voor check of veld bestaat in damo.
# veldnaam checks helemaal uitgezet. Eigenlijk niet meer nodig.

# v1.5 notes
# 23-01-2017, B. Kropf
# Domein ook koppelen aan alle subtypes indien aanwezig. -> functie str_subtypes()
# wildcard: "WS_*" verwijderd omdat nu ook niet-WS_velden in spreadsheet kunnen voorkomen. -> listFields() in functie add_fields()
...
def str_subtypes(fc):
    '''Return subtype string voor input Assign Field functie.'''
    ...
    for stcode, stdict in list(subtypes.items()):
        for stkey in list(stdict.keys()):
            if stkey == 'Name':
                d_subtype[stcode] = stdict[stkey]

    l = ["'{}: {}'".format(key,value) for key, value in d_subtype.iteritems()]
    subtype_code = ";".join(l)
    return subtype_code
...
##            raise Exception('Veldnaam %s onjuist! Veldnaam moet in UPPERCASE, enzo...'%fld)
    ...
    for fc in l_fc:
        for fld in arcpy.ListFields(fc): # wildcard: "WS_*" verwijderd omdat nu ook niet-WS_velden in spreadsheet voorkomen.
...
def create_domains(d_fields,contoleer_domeinwaarden):
    ...
    for srow in scur:
        ...
        if domein != None:
            if domein in lst_domeinen_uit_tabVelden: # controleer of domein uit tabblad "domeinen" ook is opgegeven in tabblad "velden"
                ...
                elif domein in l_domains_damo and domein != None and field_type != None and contoleer_domeinwaarden == 'true':
    ...
    for domein, code, beschrijving in domeinwaarden:
        if domein in lst_domein_te_vullen:
            ...
            if not [domein,code,beschrijving] in l_damo_domeinwaarden and contoleer_domeinwaarden == 'true':
    ...
    for domein, code, beschrijving in l_damo_domeinwaarden:
        if not [domein,code,beschrijving] in domeinwaarden and domein in lst_domeinen_uit_tabVelden and contoleer_domeinwaarden == 'true':
            i+=1
...
def assign_domains(d_fields):
    ...
    for fieldnr,f in sorted(d_fields.iteritems()):
        if not (f["fc"],f["veldnaam"].upper(),f["domein"]) in l_fld_damo_domein and f["domein"] != None: # veldnaam.upper() omdat oracle alles in uppercase geeft! zie note 1.4
            print_log("\ndomein %s niet gekoppeld aan veld [%s] van %s"%(f["domein"],f["veldnaam"],f["fc"]),"i")
            try:
                print_log("\tprobeer domein te koppelen...","i")
                arcpy.AssignDomainToField_management (f["fc"], f["veldnaam"], f["domein"])
                print_log("\tsucces!","i")
                # handle subtypes
                desc = arcpy.Describe(f["fc"])
                subtypeFld = desc.subtypeFieldName
                if subtypeFld != "":
                    subtype_code = str_subtypes(f["fc"])
                    print_log("Subtypes gevonden! {}".format(subtype_code),"i")
                    print_log("\tprobeer domein te koppelen aan subtypes...".format(subtype_code),"i")
                    arcpy.AssignDomainToField_management (f["fc"], f["veldnaam"], f["domein"],subtype_code)
                    print_log("\tsucces!","i")
            except Exception as e:
                print_log(e,"e")
...
def main():
    ...
    if check_domain == 'true':
        ...
        create_domains(d_fields,contoleer_domeinwaarden)
...
if __name__ == '__main__':
    ...
    contoleer_domeinwaarden = sys.argv[4] # wel of niet domeinen wegschrijven en controleren op waarden
